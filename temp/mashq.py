...
# <h1>Asosiy sahifaaa....</h1>
# <a href="{% url 'profil' %}">profil</a> 

#  {% for i in messages %}

#      <h3 style="background-color: aqua; color: blue;">
#          {{i}}
#      </h3>
...
#  </h3>
    ...
    # reply = models.ForeignKey('chat.Messages', on_delete=models.SET_NULL, null=True, blank=True)

#  def __str__(self) -> str:
#         return self.name if self.name else 'Friends' 
...
#         if azo:
#             azo=Chat.objects.get(id=azo)
#             azo.members.add(request.user)
#         a=[]
#         for i in hammasi:
...
#             hammasi = a
#         if search:
#             hammasi = Chat.objects.filter(name__icontains = search)
#         hammasi = hammasi[::-1]
#         context = {'chats':hammasi, 'chat':chat}
...
#         rasm = MessagesForm()
#         if request.POST:
#             rasm = MessagesForm(request.POST, files=request.FILES)
#             if rasm.is_valid():
#                 Messages.objects.create(
#                     rasm=rasm.cleaned_data['rasm']
#                 )  
...
#         hammasi = Chat.objects.all()
		...
		# 	{% csrf_token %}
		# 	{{rasm.as_p}}
		# 	<input name="rasm" value="{{chat.id}}" placeholder="rasm kriritnng" type="file">
